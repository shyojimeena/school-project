...
from authapp.models import SavedResource, TopicFollow
...
def topic_home(request, slug):
    ...
    headline = """Learn """ + str(current_topic.name).capitalize() + """ - from experts and online resources"""
    topics = Topic.objects.filter(resource__title__isnull=False).distinct().order_by('name')
    ...
    ctx['resourcetypes'] = OrderedDict(resourcetypes)
...
class ResourceTopicListView(SetHeadlineMixin, SidebarMixin, ListView):
    ...
    def get_queryset(self):
        ...
        self.headline = 'All ' + unicode_or_str(topic.name).capitalize() +' Resources'
        if res_type:
            res_type = get_object_or_404(ResourceType, slug=res_type)
            resources = resources.filter(resource_type=res_type)
            self.headline = unicode_or_str(topic.name).capitalize() +' Resources' + ' (' + unicode_or_str(res_type.name) + 's)'
